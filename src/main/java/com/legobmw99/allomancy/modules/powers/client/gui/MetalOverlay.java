...
public final class MetalOverlay implements IGuiOverlay {

    private static final ResourceLocation METER_TEXTURE = Allomancy.rl("textures/gui/overlay/meter.png");
    private static final int OUTLINE_COLOR = 255 << 24 | (0x55 + 0x10) << 16 | (0x55 + 0x10) << 8 | (0x55 + 0x10);
    ...
    private static void blit(GuiGraphics graphics,
                             ...
                             int uWidth,
                             int vHeight) {
        graphics.blit(METER_TEXTURE, x, y, 0, uOffset, vOffset, uWidth, vHeight, 128, 128);
    }
        ...
        player.getCapability(AllomancerCapability.PLAYER_CAP).ifPresent(data -> {
            ...
            for (Metal mt : Metal.values()) {
                if (data.hasPower(mt)) {
                    ...
                    // Draw the bars first
                    blit(guiGraphics, xCorner + 1, renderY + 5 + metalY, 7 + (6 * i), 1 + metalY, 3, 10 - metalY);
                    // Draw the gauges second, so that highlights and decorations show over the bar.
                    blit(guiGraphics, xCorner, renderY, 0, 0, 5, 20);
                    // Draw the fire if it is burning
                    if (data.isBurning(mt)) {
                        int frameCount = (currentFrame + i) % 4;
                        var frame = BURNING_FRAMES[frameCount];
                        blit(guiGraphics, xCorner, renderY + 4 + metalY, frame.x, frame.y, 5, 3);
                    }

                    if (highlight == mt) {
                        guiGraphics.renderOutline(xCorner - 1, renderY - 1, 7, 21, OUTLINE_COLOR);
                    }
                }
