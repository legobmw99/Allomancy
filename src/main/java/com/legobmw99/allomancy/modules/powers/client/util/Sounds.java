...
public class Sounds {
    public static void soundForBurnChange(boolean burning) {
        if (burning) {
            Minecraft.getInstance().player.playSound(
                    SoundEvent.createFixedRangeEvent(new ResourceLocation("item.flintandsteel.use"), 1f), 1, 5);
        } else {
            Minecraft.getInstance().player.playSound(
                    SoundEvent.createFixedRangeEvent(new ResourceLocation("block.fire.extinguish"), 1f), 1, 4);
        }
    }
    ...
    public static void spawnParticleForSound(Player player, SoundInstance sound) {
        ...
        if (soundName.contains("entity") || soundName.contains("step")) {
            ...
            Minecraft.getInstance().particleEngine.createParticle(new SoundParticleData(sound.getSource()), posX +
                                                                                                            (Math.sin(
                                                                                                                    Math.toRadians(
                                                                                                                            player.getYHeadRot())) *
                                                                                                             -.7d),
                                                                  posY + .2, posZ + (Math.cos(
                            Math.toRadians(player.getYHeadRot())) * .7d), motionX, motionY, motionZ);
        }
    }
}