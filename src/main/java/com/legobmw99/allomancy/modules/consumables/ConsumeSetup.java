...
public final class ConsumeSetup {
    ...
    public static final Supplier<RecipeSerializer<VialItemRecipe>> VIAL_RECIPE_SERIALIZER =
            RECIPES.register("vial_filling", () -> new RecipeSerializer<>(RecordCodecBuilder.mapCodec(
                    instance -> instance
                            .group(Ingredient.CODEC.fieldOf("base").forGetter(VialItemRecipe::getBase))
                            .apply(instance, VialItemRecipe::new)), StreamCodec.composite(
                    Ingredient.CONTENTS_STREAM_CODEC, VialItemRecipe::getBase, VialItemRecipe::new)));
    ...
    private static void onModifyComponents(final ModifyDefaultComponentsEvent event) {
        ...
        event.modify(Items.GOLDEN_APPLE, (builder, ctx, item) -> builder.set(FLAKE_STORAGE.get(), gold));
        event.modify(Items.GOLDEN_CARROT, (builder, ctx, item) -> builder.set(FLAKE_STORAGE.get(), gold));
        event.modify(Items.ENCHANTED_GOLDEN_APPLE, (builder, ctx, item) -> builder.set(FLAKE_STORAGE.get(), gold));
    }
}
