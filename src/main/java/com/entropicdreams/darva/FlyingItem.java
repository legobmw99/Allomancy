...
public class FlyingItem extends EntityThrowable {
	...
	public FlyingItem(World par1World, EntityLivingBase par2EntityLivingBase,
			EntityItem carried) {
	...
	public void onUpdate() {
		PacketDispatcher.sendPacketToAllInDimension(PacketHandler.updateIcon(
				this.stack.itemID, this.entityId, motionX, motionY, motionZ),
				this.dimension);
		...
		if (!this.worldObj.isRemote) {
			if (mop.entityHit == null) {
				this.worldObj.spawnEntityInWorld(new EntityItem(this.worldObj,
						mop.blockX, mop.blockY, mop.blockZ, this.stack));
			} else {
				if (this.thrower.equals(mop.entityHit)) {
					// Player hit themselves, using iron pulling most likely.
					// give them the item.
					thrower.inventory.addItemStackToInventory(stack);
				}
				mop.entityHit.attackEntityFrom(DamageSource
						.causeIndirectMagicDamage(thrower, mop.entityHit), 7);
			}
		}
		this.setDead();
	}
